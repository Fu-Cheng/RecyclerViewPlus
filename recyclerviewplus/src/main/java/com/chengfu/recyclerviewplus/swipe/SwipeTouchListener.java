package com.chengfu.recyclerviewplus.swipe;
...
public class SwipeTouchListener implements RecyclerView.OnItemTouchListener, OnActivityTouchListener {
	private static final String TAG = "RecyclerTouchListener";
	final Handler handler = new Handler();
	Activity act;
	List<Integer> unSwipeableRows;
	ArrayList<View> conflictViews;
	/*
	 * independentViews are views on the foreground layer which when clicked,
	 * act "independent" from the foreground ie, they are treated separately
	 * from the "row click" action
	 */
	...
	private int bgWidth = 1, bgWidthLeft = 1; // 1 and not 0 to prevent dividing
												// by zero
	...
	public SwipeTouchListener(Activity a, RecyclerView recyclerView) {
		this.act = a;
		...
		conflictViews = new ArrayList<View>();
		...
		rView.addOnScrollListener(new RecyclerView.OnScrollListener() {
			...
			public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
				/**
				 * This will ensure that this RecyclerTouchListener is paused
				 * during recycler view scrolling. If a scroll listener is
				 * already assigned, the caller should still pass scroll changes
				 * through to this listener.
				 */
				setEnabled(newState != RecyclerView.SCROLL_STATE_DRAGGING);

				/**
				 * This is used so that clicking a row cannot be done while
				 * scrolling
				 */
	...
	public SwipeTouchListener setClickable(OnRowClickListener listener) {
		this.clickable = true;
		this.mRowClickListener = listener;
		return this;
	}

	public SwipeTouchListener setClickable(boolean clickable) {
		this.clickable = clickable;
		return this;
	}

	public SwipeTouchListener setLongClickable(boolean vibrate, OnRowLongClickListener listener) {
	...
	public SwipeTouchListener setLongClickable(boolean longClickable) {
		this.longClickable = longClickable;
		return this;
	}

	public SwipeTouchListener setIndependentViews(Integer... viewIds) {
		this.independentViews = new ArrayList<Integer>(Arrays.asList(viewIds));
		return this;
	}

	public SwipeTouchListener setUnClickableRows(Integer... rows) {
	...
	public SwipeTouchListener setSwipeable(int foregroundID, int backgroundID,
	...
	/*
	 * public RecyclerTouchListener setLeftToRightSwipeable(int foregroundID,
	 * int backgroundID, OnSwipeOptionsClickListener listener) {
	 * this.swipeableLeftOptions = true; if (fgViewID != 0 && foregroundID !=
	 * fgViewID) throw new IllegalArgumentException(
	 * "foregroundID does not match previously set ID"); fgViewID =
	 * foregroundID; bgViewIDLeft = backgroundID; this.mBgClickListenerLeft =
	 * listener;
	 * 
	 * if (act instanceof RecyclerTouchListenerHelper)
	 * ((RecyclerTouchListenerHelper) act).setOnActivityTouchListener(this);
	...
	public SwipeTouchListener setSwipeable(boolean value) {
	...
	public SwipeTouchListener setSwipeOptionViews(Integer... viewIds) {
		this.optionViews = new ArrayList<Integer>(Arrays.asList(viewIds));
		return this;
	}

	public SwipeTouchListener setUnSwipeableRows(Integer... rows) {
		this.unSwipeableRows = new ArrayList<Integer>(Arrays.asList(rows));
		return this;
	}

	public SwipeTouchListener setEventConflictView(View... views) {
		this.conflictViews = new ArrayList<View>(Arrays.asList(views));
		return this;
	}
	...
	public void openSwipeOptions(int position) {
		if (!swipeable || rView.getChildAt(position) == null || unSwipeableRows.contains(position))
			return;
		if (bgWidth < 2) {
	...
	private boolean handleTouchEvent(MotionEvent motionEvent) {
		...
		case MotionEvent.ACTION_DOWN: {
			...
			/*
			 * check for every child (row) in the recycler view whether the
			 * touched co-ordinates belong to that respective child and if it
			 * does, register that child as the touched view (touchedView)
			 */
			...
			if (touchedView != null) {
				...
				if (swipeable) {
					 ...
					 * block is to register fgPartialViewClicked status -
			...
			/*
			 * If options menu is shown and the touched position is not the same
			 * as the row for which the options is displayed - close the options
			 * menu for the row which is displaying it (bgVisibleView and
			 * bgVisiblePosition is used for this purpose which registers which
			 * view and which position has it's options menu opened)
			 */
			x = (int) motionEvent.getRawX();
			y = (int) motionEvent.getRawY();
		...
		// When finger is lifted off the screen (after clicking, flinging,
		// swiping, etc..)
		case MotionEvent.ACTION_UP: {
			if (conflictViews != null) {
				for (View v : conflictViews) {
					v.setEnabled(true);
				}
			}
			handler.removeCallbacks(mLongPressed);
			...
			// swipedLeft and swipedRight are true if the user swipes in the
			// respective direction (no conditions)
			boolean swipedLeft = false;
			boolean swipedRight = false;
			/*
			 * swipedLeftProper and swipedRightProper are true if user swipes in
			 * the respective direction and if certain conditions are satisfied
			 * (given some few lines below)
			 */
			...
			/*
			 * If the user has swiped more than half of the width of the options
			 * menu, or if the velocity of swiping is between min and max fling
			 * values "proper" variable are set true
			 */
			if (Math.abs(mFinalDelta) > bgWidth / 2 && isFgSwiping) {
				...
				float absVelocityX = Math.abs(velocityX);
				float absVelocityY = Math.abs(mVelocityTracker.getYVelocity());
				if (minFlingVel <= absVelocityX && absVelocityX <= maxFlingVel && absVelocityY < absVelocityX
						&& isFgSwiping) {
			...
			///////// Manipulation of view based on the 4 variables mentioned
			///////// above ///////////

			// if swiped left properly and options menu isn't already visible,
			// animate the foreground to the left
			if (swipeable && !swipedRight && swipedLeftProper && touchedPosition != RecyclerView.NO_POSITION
				...
				final View downView = touchedView; // touchedView gets null'd
			...
			// else if swiped right properly when options menu is visible, close
			// the menu and bring the foreground
			...
			// else if swiped left incorrectly (not satisfying the above
			// conditions), animate the foreground back to
			...
			// else if swiped right incorrectly (not satisfying the above
			// conditions), animate the foreground to
			...
			// This case deals with an error where the user can swipe left, then
			// right
			// really fast and the fg is stuck open - so in that case we close
			// the fg
			else if (swipeable && swipedRight && !bgVisible) {
				// cancel
				animateFG(touchedView, Animation.CLOSE, ANIMATION_STANDARD);
			...
			// This case deals with an error where the user can swipe right,
			// then left
			// really fast and the fg is stuck open - so in that case we open
			// the fg
			else if (swipeable && swipedLeft && bgVisible) {
				// cancel
				animateFG(touchedView, Animation.OPEN, ANIMATION_STANDARD);
			...
			// if clicked
				...
				// if partial foreground view is clicked (see ACTION_DOWN) bring
				// foreground back to original position
				// bgVisible is true automatically since it's already checked in
				// ACTION_DOWN block
		...
		case MotionEvent.ACTION_MOVE: {
			...
			/*
			 * isFgSwiping variable which is set to true here is used to alter
			 * the swipedLeft, swipedRightProper variables in "ACTION_UP" block
			 * by checking if user is actually swiping at present or not
			 */
			...
			// This block moves the foreground along with the finger when
			// swiping
			if (swipeable && isFgSwiping && !unSwipeableRows.contains(touchedPosition)) {
				...
				if (deltaX < touchSlop && !bgVisible) {
					...
					// if ((Math.abs(translateAmount) > bgWidth ? -bgWidth :
					// translateAmount) <= 0) {
					// swipe fg till width of bg. If swiped further, nothing
					// happens (stalls at width of bg)
				...
				// if fg is being swiped right
					...
					if (bgVisible) {
						...
						// swipe fg till it reaches original position. If swiped
						// further, nothing happens (stalls at 0)
						fgView.setTranslationX(translateAmount > 0 ? 0 : translateAmount);

						// fades all the fadeViews gradually to 0 alpha as
						// dragged
						if (fadeViews != null) {
							for (int viewID : fadeViews) {
								touchedView.findViewById(viewID).setAlpha(1 - (Math.abs(translateAmount) / bgWidth));
			...
			// moves the fg slightly to give the illusion of an "unswipeable"
			// row
				...
				if (deltaX < touchSlop && !bgVisible) {
					...
					// if (fadeViews != null) {
					// for (int viewID : fadeViews) {
					// touchedView.findViewById(viewID).setAlpha(1 -
					// (Math.abs(translateAmount) / bgWidth));
					// }
					// }
				}
				return true;
	...
	/**
	 * Gets coordinates from Activity and closes any swiped rows if touch
	 * happens outside the recycler view
	 */
	@Override
